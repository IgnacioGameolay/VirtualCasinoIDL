...
void InicializarBarajaBL(TipoBaraja *baraja) { //Inicializador de baraja
...
Stack* MezclarBarajaBL(List* listaCartas) { //Mezclador de barajas
	// Crear mapa y stack
...
int CalcularPuntaje(List* mano){ //Calcula el puntaje recorriendo la lista y sumando los valores
	int puntaje = 0;
	TipoCarta* carta = list_first(mano);
	while(carta != NULL){
		puntaje += carta->valor;
...
void MostrarReglasBlackjack(){  // Muestra las reglas del blackjack
...
void menuBL(){  //Muestra menu del blackjack
	printf("========================================\n");
...
void determinarGanador(int puntajeJugador, int puntajeDealer, int apuesta, int* chipCount){
	if (puntajeJugador > 21){   //Si el jugador pasa de 21
		printf("Te has pasado de 21, pierdes la apuesta.\n");
		*chipCount -= apuesta;
	}
	else if (puntajeDealer > 21 && puntajeJugador <= 21){  //Si el dealer pasa de 21 y el jugador no
		printf("El crupier se ha pasado de 21, ganaste la apuesta.\n");
		*chipCount += apuesta * 2;
	}

	else if (puntajeJugador > puntajeDealer && puntajeJugador < 22){ //Si el jugador tiene mas puntos que el dealer
		printf("Ganaste la apuesta.\n");
		*chipCount += apuesta * 2;
	}

	else if (puntajeJugador < puntajeDealer && puntajeDealer < 22){ //Si el dealer tiene mas puntos que el jugador
		printf("Perdiste la apuesta.\n");
		*chipCount -= apuesta;
	}
	else if (puntajeJugador == puntajeDealer && puntajeJugador < 22 && puntajeDealer < 22){ //Si el jugador y el dealer tienen la misma cantidad de puntos
		printf("Es un empate, se devuelve la apuesta.\n");
		*chipCount += apuesta;
	}
}

void jugarBL(int *chipCount){  //Juego Blackjack
	int apuesta;
	char respuesta;
	printf("================================\n");
	...
	while (respuestaJugador == 's' && puntajeJugador < 21){ //Ciclo de juego
		list_pushBack(cartasJugador, (TipoCarta*)SacarCartaBL(pilaCartas));
		puntajeJugador = CalcularPuntaje(cartasJugador);
		printf("Mano del jugador: ");
		MostrarMano(cartasJugador);
...
int BlackjackGame(int *chipCount){ //Menu del juego
	char respuesta;
